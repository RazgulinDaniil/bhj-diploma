 ...
 * */
 class TransactionsPage {
   ...
   * */
  registerEvents() {
    const accountRemove = document.querySelector(".content-header");
    accountRemove.addEventListener('click', (e) => {
      if (e.target.classList.contains('remove-account')) {
        this.removeAccount();
      }
    });
    const transactionRemove = document.querySelector('.content');
  ...
  render(options) {
    ...
    const data = {
      addUrl: '?account_id=' + options.account_id
    };
  ...
  renderTransactions(data) {
    ...
    if (data && data.length === 0) {
      ...
      content.innerHTML = "";
      data.forEach(elem => {
        const code = this.getTransactionHTML(elem);
